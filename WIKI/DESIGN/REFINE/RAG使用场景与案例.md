...
## 接口设计
RAG 接口设计逻辑：传入一组关键词，后台通过embedding相似度返回外部招投标数据和用户本地数据的交集
    入参：list[string]， 返回：项目名 | 项目概要（总结） | 日期 | url | 用户名（供应商或设计机构） | 甲方名 | 

    RAG整理（从用户请求到返回）全链路设计逻辑：
        背景：用户输入问题后，如果只通过google的公网搜索，无法找到精准的高质量数据，因此需要RAG以内部数据进行补充，
        将外部和内部知识打通，合并信息提供给LLM，LLM综合后，返回更高质量的回答。
    请求和数据分层：
        langgraph流程内，在  thinking_startup_stage/generate_query 之后，通过现有路由策略，
        进行 web_research + rag_search 的搜索，此阶段内部链路调用 query_rag_rest 之后，
        接口返回 （项目名 | 项目概要（总结） | 日期 | url | 用户名（供应商或设计机构） | 甲方名 | ）等信息给到LLM（reflection），
        但LLM最终输出的答案，只需要输出 （项目名 | 日期 | url）给用户（enhanced_report_instructions的提示词需要调整）。
