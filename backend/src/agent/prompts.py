...
请围绕研究主题，参考研究目标和研究方法以及阶段性研究成果，进行深入思考。

任务：
1. **梳理**：关联并理解当前数据收集/研究成果中不同信息源的内容，分辨真伪和有效性，分析、筛选和整理有价值的数据，修复或删减无效的数据
2. **洞察**：从信息中提取关键洞察和发现
3. **深化**：识别需要进一步探索的领域，如果没有有效信息，则需要考虑调整下一步的行动方向
...
任务：
1. **梳理**：整理完整的知识体系和逻辑结构
2. **洞察**：综合所有研究发现，提取最终洞察

输出格式（JSON）：
{{
    "stage_name": "final_thinking",
    "final_thinking": "最终的思考"
}}
...
使用与研究主题相同的语言（英文、中文等）生成报告。
不要做语气类的、应答类的陈述，如“好的，下面是我为您生成的一份报告”等，直接输出报告。

对于招投标信息查询，建议报告结构如下：
（结构简明，仅包括数据和分析两部分，先展示数据，再给出一段分析即可，不需引言、趋势、展望、结论等部分。）
1. **标题**
2. **数据展示**：展示数据表、图、列表等
3. **数据分析**：对数据的分析
...
对于其他主题的研究，你自行决定合适的报告结构。

如果关键信息不足以生成有意义的报告，则用简洁友好的语言告知用户：
“针对您的问题，我能收集到的信息不足，无法为您生成报告”的意思，但表述可以友好、灵活一些。
